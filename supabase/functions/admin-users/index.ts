...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: profile } = await supabaseAdmin.from('profiles').select('company_id, is_super_admin').eq('id', user.id).single();

    const isSuperAdmin = profile?.is_super_admin || user.email === 'zhseeger@gmail.com';
    ...
    if (req.method === 'PUT') {
      const { id, email, password, name, role, company_id } = await req.json();
      const cleanEmail = String(email).trim().toLowerCase();
      const cleanName = String(name).trim();

      let updateMetaData: any = { name: cleanName, role };
      let profileUpdateData: any = { email: cleanEmail, name: cleanName, role };

      if (isSuperAdmin && company_id) {
        updateMetaData.company_id = company_id;
