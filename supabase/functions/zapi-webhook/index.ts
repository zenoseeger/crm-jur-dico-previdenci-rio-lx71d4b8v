...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    else if (payload.document) content = payload.document.caption || payload.document.fileName || '[Documento recebido]';
    ...
    // Connection validation: Receiving webhooks means we are connected
    ...
    if (leadId) {
      ...
      // Invoke ai-observer asynchronously to handle qualification and auto-reply
      try {
